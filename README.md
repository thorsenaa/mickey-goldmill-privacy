# Privacy Policy

**FocusRep — Workout Tracker**
Last updated: May 26, 2026

---

## What We Collect

FocusRep collects only what is necessary to provide the app's core functionality.

**Account information**
When you sign in with Apple, we receive your name and email address (or Apple's anonymized relay address if you choose to hide your email). This is used solely to create and identify your account.

**Workout data**
We store the workout templates, session logs, schedules, and streak data you create in the app. This data is saved on your device and synced to our cloud database (Supabase) so it can be restored if you reinstall the app or switch devices.

## What We Do Not Collect

- Location data
- Health or fitness data from HealthKit or any other health platform
- Device identifiers or advertising IDs
- Analytics or usage tracking
- Crash reports sent to third parties

## How Your Data Is Used

Your data is used exclusively to operate FocusRep. We do not sell, share, or disclose your data to third parties for any purpose.

## Data Storage

Your workout data is stored on your device using local storage and synced to a secure cloud database provided by Supabase (supabase.com). Data is protected using Row Level Security — only you can access your own data.

## Data Deletion

To delete your data, contact us at the email below and we will remove your account and all associated data within 30 days.

## Children

FocusRep is not directed at children under 13. We do not knowingly collect data from children under 13.

## Changes to This Policy

If we make material changes to this policy, we will update the date at the top of this page.

## Contact

For privacy questions or data deletion requests:
**thorsen120@gmail.com**
